...
void main() {
  ...
  // Calculate k
  float ka = max(dot(transformed_normal, light_dir), 0.0);
  // Calculate diffuse
  vec4 diffuse = ka * (diffuse_reflection * light_colour);
  ...
  // Calculate k
  float ks = pow(max(dot(transformed_normal, half_vector), 0.0), shininess);
  // Calculate specular
