...
bool load_content() {
  ...
  meshes["cylinder"] = mesh(geometry_builder::create_cylinder(20, 10));
  ...
  // Set lighting values
  // Position (-25, 10, -10)
  light.move(vec3(-25.0f, 10.0f, -10.0f));
  // Light colour white
...
bool update(float delta_time) {
  ...
  if (glfwGetKey(renderer::get_window(), 'W')) {
	  light.move(vec3(0.0f, 0.0f, -5.0f) * delta_time);
  }
  if (glfwGetKey(renderer::get_window(), 'S')) {
	  light.move(vec3(0.0f, 0.0f, 5.0f) * delta_time);
  }
  if (glfwGetKey(renderer::get_window(), 'A')) {
	  light.move(vec3(-5.0f, 0.0f, 0.0f) * delta_time);
  }
  ...
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_UP)) {
	 light.rotate(vec3(-pi<float>() * delta_time, 0.0f, 0.0f));
  }
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_DOWN)) {
	  light.rotate(vec3(pi<float>() * delta_time, 0.0f, 0.0f));
  }
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_LEFT)) {
	  light.rotate(vec3(0.0f, pi<float>() * delta_time, 0.0f));
  }
